...
async def handle_ranked_submission(interaction: discord.Interaction, modal: RankedOrderModal) -> None:
    ...
    if starting_rank is None or desired_rank is None:
        await interaction.followup.send(
            ...
            "`Diamond II` or `Legendary I`, then click Fill Order Details again.",
            ephemeral=True,
        )
        return
    ...
    if distance is None or distance <= 0:
        await interaction.followup.send(
            "Your desired rank needs to be higher than your starting rank. Click Fill Order Details again to retry.",
            ephemeral=True,
        )
        return

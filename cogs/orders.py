...
async def handle_ranked_submission(
    interaction: discord.Interaction,
    modal: RankedOrderModal,
) -> None:
    ...
    breakdown = calculate_rank_price(
        starting_rank,
        desired_rank,
        p11_count,
        modal.is_duo_carry,
    ...
    order_type = (
        "Rank Carry (Duo)"
        if modal.is_duo_carry
        else "Rank Boost (Solo)"
    )
    ...
    if player is not None:
        ...
        summary_lines.append(
            f"**Account Lookup** — Unavailable ({api_error})"
        )
    ...
    if modal.notes.value:
        summary_lines.append(
            f"**Notes** — {modal.notes.value}"
        )

    if breakdown is not None:
        summary_lines.append(
            f"**Price** — {breakdown.formatted(CURRENCY_SYMBOL)}"
        )
...
class PrestigeOrderModal(discord.ui.Modal):
    def __init__(self, is_duo_carry: bool) -> None:
        title = (
            "Prestige Carry Order"
            if is_duo_carry
            else "Prestige Boost Order"
        )
...
async def handle_prestige_submission(
    interaction: discord.Interaction,
    modal: PrestigeOrderModal,
) -> None:
    await interaction.response.defer(ephemeral=True, thinking=True)
    ...
    if desired_level is None:
        await interaction.followup.send(
            "I couldn't recognize that Prestige level. "
            "Please use a format like `P3` and try again.",
            ephemeral=True,
        )
        return
    ...
    if player is None:
        await interaction.followup.send(
            "I couldn't look up that account, so I can't calculate an exact price "
            f"({api_error}). Please double check the player tag and try again.",
            ephemeral=True,
        )
        return

    brawler = find_brawler(
        player,
        modal.brawler_name.value,
    )

    if brawler is None:
        await interaction.followup.send(
            f"**{player.get('name', 'That account')}** doesn't seem to have "
            f"a brawler named \"{modal.brawler_name.value}\" unlocked. "
    ...
    breakdown = calculate_prestige_price(
        current_trophies,
        desired_level,
        modal.is_duo_carry,
    )
    ...
    if breakdown is None:
        ...
        await interaction.followup.send(
            f"**{brawler.get('name', modal.brawler_name.value)}** is already "
            f"at Prestige {current_prestige} ({current_trophies:,} trophies) "
            f"on that account, which is at or above P{desired_level}. "
    ...
    summary_lines = [
        ...
        f"({trophies_required_for_prestige(desired_level):,} trophies)",
        f"**Type** — {order_type}",
        f"**Payment Method** — {modal.payment_method.value}",
    ]
...
async def _on_order_other_select(interaction: discord.Interaction) -> None:
    ...
    await interaction.response.send_modal(
        OtherOrderModal(option_key=values[0])
    )
...
def other_panel_callbacks() -> CallbackMap:
    return {
        "order_other_select": _on_order_other_select,
    }
...
class Orders(commands.Cog):
    ...
    @app_commands.command(
        name="ranked",
        description="Post the ranked boosting order panel.",
    )
    @staff_only()
    async def ranked(self, interaction: discord.Interaction) -> None:
        view = load_layout_view(
            EMBEDS_DIR / "ranked.json",
            callbacks=ranked_panel_callbacks(),
            timeout=None,
        )
    ...
    @app_commands.command(
        name="prestiges",
        description="Post the prestige boosting order panel.",
    )
    @staff_only()
    async def prestiges(self, interaction: discord.Interaction) -> None:
        view = load_layout_view(
            EMBEDS_DIR / "prestiges.json",
            callbacks=prestige_panel_callbacks(),
            timeout=None,
        )
    ...
    @app_commands.command(
        name="other",
        description="Post the other services order panel.",
    )
    @staff_only()
    async def other(self, interaction: discord.Interaction) -> None:
        view = load_layout_view(
            EMBEDS_DIR / "other.json",
            callbacks=other_panel_callbacks(),
            timeout=None,
        )
